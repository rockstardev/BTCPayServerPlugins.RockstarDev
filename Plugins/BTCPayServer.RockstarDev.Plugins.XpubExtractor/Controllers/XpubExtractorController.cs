...
public class XpubExtractorController : Controller
{
    ...
    // This is a websocket endpoint that is used by javascript to fetch info
    ...
    public async Task<IActionResult> VaultBridgeConnection(string cryptoCode = null,
        [ModelBinder(typeof(WalletIdModelBinder))] WalletId walletId = null)
    {
        if (!HttpContext.WebSockets.IsWebSocketRequest) return NotFound();
