...
public class WalletSweeperService : IPeriodicTask
{
    private readonly PluginDbContextFactory _dbContextFactory;
    private readonly ILogger<WalletSweeperService> _logger;

    public WalletSweeperService(
    ...
    {
        _logger.LogInformation("WalletSweeper: Starting periodic check");
        ...
        try
        {
            ...
            _logger.LogInformation($"WalletSweeper: Found {configs.Count} enabled configurations");
            ...
            foreach (var config in configs)
            {
                ...
                {
                    _logger.LogError(ex, $"WalletSweeper: Error processing configuration {config.Id} for store {config.StoreId}");
                }
            }
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "WalletSweeper: Error in periodic task");
        }
    }

    private async Task ProcessConfiguration(SweepConfiguration config, CancellationToken cancellationToken)
    {
        _logger.LogInformation($"WalletSweeper: Processing config {config.Id} for store {config.StoreId}");
        ...
        if (config.LastSweepDate.HasValue)
        {
            ...
            {
                _logger.LogInformation($"WalletSweeper: Skipping - only {daysSinceLastSweep:F1} days since last sweep (interval: {config.IntervalDays} days)");
                return;
            }
        }
        ...
        _logger.LogInformation($"WalletSweeper: Store {config.StoreId} balance: {currentBalance} BTC");

        // Check minimum balance threshold
        if (currentBalance < config.MinimumBalance)
        {
            _logger.LogInformation($"WalletSweeper: Balance {currentBalance} is below minimum {config.MinimumBalance}, skipping");
            return;
        }
    ...
    private async Task<decimal> GetWalletBalance(string storeId, CancellationToken cancellationToken)
    {
        ...
        _logger.LogWarning($"WalletSweeper: GetWalletBalance not implemented yet for store {storeId}");
        return 0m;
    }
    ...
    {
        _logger.LogInformation($"WalletSweeper: Executing sweep for store {config.StoreId}, trigger: {triggerType}");
        ...
        try
        {
            ...
            _logger.LogInformation($"WalletSweeper: Sweep successful! Amount: {sweepAmount} BTC, Fee: {estimatedFee} BTC");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, $"WalletSweeper: Sweep failed for store {config.StoreId}");
